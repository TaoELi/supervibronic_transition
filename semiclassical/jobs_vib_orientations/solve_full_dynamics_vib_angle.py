...
# additional parameters to control the vibrational subsystem
v_dipole_angle_thres = params["v_dipole_angle_thres"]
orientation_vib = np.cos(np.linspace(-v_dipole_angle_thres, v_dipole_angle_thres, Ne))
print("orientation_vib = ", orientation_vib)
...
def func_dynamics(t, state):
    ...
    current_muv = dv * np.sum(xd * orientation_vib)
    ...
    dmudr = dv * orientation_vib
    dxbdt = pb
    # dpbdt = -omega_v ** 2 * xb - lambda_c * omega_c * xc * dmudr - gamma_v * np.sum(xd) - lambda_c ** 2 * (current_muv + current_mue_single * Ne) * dmudr
    dpbdt = -omega_v ** 2 * xb # - lambda_c * omega_c * xc * dmudr - lambda_c ** 2 * (current_muv + current_mue_single * Ne) * dmudr
    dxddt = pd
    # dpddt = -omega_v_dark ** 2 * xd - gamma_v * xb
    dpddt = -omega_v_dark ** 2 * xd - lambda_c * omega_c * xc * dmudr - lambda_c ** 2 * (current_muv + current_mue_single * Ne) * dmudr
    #dpddt[1:-1] -= gamma_v * xd[0:-2] + gamma_v * xd[2:]
    #dpddt[0] -= gamma_v * xd[1] + gamma_v * xd[-1]
    #dpddt[-1] -= gamma_v * xd[-2] + gamma_v * xd[0]
...
for t, y in zip(t_values, y_values):
    ...
    # recalculate e_vib
    xb = 1.0/Nv**0.5 * np.sum(xd)
    pb = 1.0/Nv**0.5 * np.sum(pd)
    e_vib = 0.5 * omega_v ** 2 * xb ** 2 + 0.5 * pb ** 2
    e_dark -= e_vib
