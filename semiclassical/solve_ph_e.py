...
Hme = np.zeros((Ne+1, Ne+1))
Mue_t = np.zeros((Ne+1, Ne+1))
Mue_p = np.zeros((Ne+1, Ne+1))
...
def wrap_initial_state(Rho_e, xc, pc):
    initial_state = np.zeros((Ne + 1) * (Ne + 1) + 2, dtype=np.complex128)
    initial_state[0] = xc
    initial_state[1] = pc
...
def decode_initial_state(initial_state):
    xc = initial_state[0]
    pc = initial_state[1]
    Rho_e = initial_state[2:].reshape((Ne+1, Ne+1))
    return xc, pc, Rho_e


# Define the initial electronic state as the ground state
Rho_e = np.zeros((Ne+1, Ne+1))
Rho_e[0, 0] = 1.0
xc, pc = 0.0, 0.0
