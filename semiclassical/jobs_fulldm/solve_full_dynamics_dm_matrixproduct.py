...
# electronic dipole orientation
e_dipole_angle_thres = params["e_dipole_angle_thres"]
e_dipole_cos_lst = np.cos(np.linspace(-e_dipole_angle_thres, e_dipole_angle_thres, Ne))
print("e_dipole_cos_lst = ", e_dipole_cos_lst)
print("<cos theta> = ", np.mean(e_dipole_cos_lst))
print("<cos theta^2> = ", np.mean(e_dipole_cos_lst**2))
print("<cos theta^3>^2 = ", np.mean(e_dipole_cos_lst**3)**2)
...
SigmaMinus = np.array([[0.0, 1.0], [0.0, 0.0]])
SigmaPlus = np.array([[0.0, 0.0], [1.0, 0.0]])
SigmaPlusSigmaMinus = np.dot(SigmaPlus, SigmaMinus)
...
Hme_expand = np.zeros((2, 2, Ne))
Mue_expand = np.zeros((2, 2, Ne))
Mue2_expand = np.zeros((2, 2, Ne))
Mue_t_expand = np.zeros((2, 2, Ne))
for j in range(Ne):
    Hme_expand[:, :, j] = Hme
    # account for the orientation of the electronic dipoles
    Mue_expand[:, :, j] = Mue * e_dipole_cos_lst[j]
    Mue2_expand[:, :, j] = Mue2 * e_dipole_cos_lst[j]**2
    Mue_t_expand[:, :, j] = Mue_t * e_dipole_cos_lst[j]

Ndim_e2 = 2*2*Ne


def wrap_initial_state(Rho_e, xc, pc, xb, pb, xd, pd):
    # the electronic density matrix is prepared as the form of a 2*2*Ne tensor
    initial_state = np.zeros(Ndim_e2 + 4 + Ndark*2, dtype=np.complex128)
    initial_state[0:Ndim_e2] = Rho_e.flatten()
    initial_state[Ndim_e2] = xc
    initial_state[Ndim_e2+1] = pc
    initial_state[Ndim_e2+2] = xb
    initial_state[Ndim_e2+3] = pb
    initial_state[Ndim_e2+4:Ndim_e2+4+Ndark] = xd
    initial_state[Ndim_e2+4+Ndark:] = pd
    return initial_state


def decode_initial_state(initial_state):
    Rho_e = initial_state[0:Ndim_e2].reshape((2, 2, Ne))
    xc = initial_state[Ndim_e2]
    pc = initial_state[Ndim_e2+1]
    xb = initial_state[Ndim_e2+2]
    pb = initial_state[Ndim_e2+3]
    xd = initial_state[Ndim_e2+4:Ndim_e2+4+Ndark]
    pd = initial_state[Ndim_e2+4+Ndark:]
...
# expand the electronic dimensions in the basis of Ne TLSs
Rho_e_expand = np.zeros((2, 2, Ne))
for j in range(Ne):
    Rho_e_expand[:, :, j] = Rho_e

xc, pc = params["xc_start"], 0.0
xb, pb = 0.0, 0.0
xd, pd = np.zeros(Ndark), np.zeros(Ndark)
initial_state = wrap_initial_state(Rho_e_expand, xc, pc, xb, pb, xd, pd)

print("Initial state is")
print("Rho_e = ", Rho_e)
print("Rho_e_expand = ", Rho_e_expand)
print("xc = ", xc)
print("pc = ", pc)
print("xb = ", xb)
print("pb = ", pb)
...
def func_dynamics(t, state):
    ...
    Rho_e_expand, xc, pc, xb, pb, xd, pd = decode_initial_state(state)
    # equations of motion
    dxcdt = pc
    # calculate the total dipole moment for Ne TLSs with the fancy Einstein summation notation :)
    current_mue = np.trace(np.einsum('ijl,jkl', Rho_e_expand, Mue_expand))
    current_muv = Nv ** 0.5 * xb * dv
    dpcdt = -omega_c**2 * xc - lambda_c * omega_c * (current_mue + current_muv) - gamma_c * pc
    Hsc_expand = Hme_expand + (omega_c * lambda_c * xc) * Mue_expand + (0.5 * lambda_c**2) * (Mue2_expand + 2.0 * Mue_expand * current_muv + 2.0 * current_mue * Mue_expand)

    # remove the double counting of the self-interaction
    #for j in range(Ne):
    #    Hsc_expand[:, :, j] -= lambda_c**2 * Mue_expand[:, :, j] * np.trace(np.dot(Rho_e_expand[:, :, j], Mue_expand[:, :, j]))

    # we now find another way to remove the double counting of the self-interaction (without the for loop, which is very slow in python)
    # first, we do the matrix multiplication of the first two dimensions of the electronic density matrix and the dipole moment matrix
    current_mue_lst = np.einsum("ijh,jk->ikh", Rho_e_expand, Mue)
    # second, we do the trace over the first two dimensions, and get an array of the dipole moment of each TLS
    current_mue_lst = np.einsum("ii...", current_mue_lst) * e_dipole_cos_lst
    current_mue_lst = current_mue_lst.reshape((1, 1, Ne))
    # third, we remove the double counting in an elegant way
    Hsc_expand -= lambda_c**2 * Mue_expand * current_mue_lst

    # add the external pulse to excite the system
    Hsc_expand += gaussian_pulse(t) * Mue_t_expand

    # calculate the time derivative of the electronic density matrix with the sloppy loop over the Ne TLSs, we need to think about how to optimize this with the fancy Einstein summation notation
    dRhoedt_expand = np.zeros((2, 2, Ne), dtype=np.complex128)
    #for j in range(Ne):
        #dRhoedt_expand[:, :, j] = -1j * np.dot(Hsc_expand[:, :, j], Rho_e_expand[:, :, j]) + 1j * np.dot(Rho_e_expand[:, :, j], Hsc_expand[:, :, j])
        # Add a Lindblad term to describe the spontaneous emission and decoherence
        #Lindblad = gamma_se * (np.dot(SigmaMinus, np.dot(Rho_e_expand[:, :, j], SigmaPlus)) - 0.5 * np.dot(SigmaPlus, np.dot(SigmaMinus, Rho_e_expand[:, :, j])) - 0.5 * np.dot(Rho_e_expand[:, :, j], np.dot(SigmaPlus, SigmaMinus)))
        #dRhoedt_expand[:, :, j] += Lindblad

    # let us think about how to optimize the above loop with the fancy Einstein summation notation
    # the first part is doable [I agree that the code is now very hard to read by a human, but it is very fast]
    Hsc_expand_switch = np.einsum("ijk->kij", Hsc_expand)
    Rho_e_expand_switch = np.einsum("ijk->kij", Rho_e_expand)
    dRhoedt_expand_switch = -1j * np.matmul(Hsc_expand_switch, Rho_e_expand_switch) + 1j * np.matmul(Rho_e_expand_switch, Hsc_expand_switch)
    dRhoedt_expand = np.einsum("kij->ijk", dRhoedt_expand_switch)
    # The Lindblad term can be written as the following magic way
    L1 = np.einsum("ij,jkh->ikh", SigmaMinus, np.einsum("ijh,jk->ikh", Rho_e_expand, SigmaPlus))
    L2 = np.einsum("ij,jkh->ikh", SigmaPlus, np.einsum("ij,jkh->ikh", SigmaMinus, Rho_e_expand))
    L3 = np.einsum("ijh,jk->ikh", Rho_e_expand, SigmaPlusSigmaMinus)
    Lindblad = gamma_se * (L1 - 0.5 * L2 - 0.5 * L3)
    dRhoedt_expand += Lindblad


    dmudr = Nv ** 0.5 * dv
    dxbdt = pb
    dpbdt = -omega_v ** 2 * xb - lambda_c * omega_c * xc * dmudr - gamma_v * np.sum(xd) - lambda_c ** 2 * (current_muv + current_mue) * dmudr
    dxddt = pd
    dpddt = -omega_v_dark ** 2 * xd - gamma_v * xb

    # Encode the equations of motion
    dstate = wrap_initial_state(dRhoedt_expand, dxcdt, dpcdt, dxbdt, dpbdt, dxddt, dpddt)
    return dstate
...
for t, y in zip(t_values, y_values):
    Rho_e_expand, xc, pc, xb, pb, xd, pd = decode_initial_state(y)
    xc_lst.append(xc)
    xb_lst.append(xb)
    xd_lst.append(np.mean(xd**2))
    Pg_lst.append(np.mean(np.real(Rho_e_expand[0, 0, :])))
    mu_current = np.real(np.trace(np.einsum('ijl,jkl', Rho_e_expand, Mue_expand))) + Nv ** 0.5 * xb * dv
    e_ph = 0.5 * omega_c**2 * (xc + lambda_c / omega_c * mu_current)**2 + 0.5 * pc**2
    e_ph_lst.append(e_ph)
    e_Hme = np.real(np.trace(np.einsum('ijl,jkl', Rho_e_expand, Hme_expand)))
