...
print("the average value of the cosine of the electronic dipole orientation is", np.mean(e_dipole_cos_lst))


# solving the full density matrix for the electronic system is very expensive, so we work with only a small value of Ne
if Ne > 10:
    print("The number of electronic two-level systems is too large, aborting the simulation")
    exit()
...
# After defining the matrices in the basis of a single TLS, we now expand them to the basis of Ne TLSs
Hme_lst = [Hme] * Ne
Mue_t_lst = [Mue_t] * Ne
Mue_lst = [Mue] * Ne
SigmaPlus_lst = [SigmaPlus] * Ne
SigmaMinus_lst = [SigmaMinus] * Ne
for j in range(Ne):
    for i in range(Ne):
        if j == i:
            continue
        elif j > i:
            Hme_lst[j] = np.kron(Identity, Hme_lst[j])
            Mue_lst[j] = np.kron(Identity, Mue_lst[j])
            Mue_t_lst[j] = np.kron(Identity, Mue_t_lst[j])
            SigmaPlus_lst[j] = np.kron(Identity, SigmaPlus_lst[j])
            SigmaMinus_lst[j] = np.kron(Identity, SigmaMinus_lst[j])
        else:
            Hme_lst[j] = np.kron(Hme_lst[j], Identity)
            Mue_lst[j] = np.kron(Mue_lst[j], Identity)
            Mue_t_lst[j] = np.kron(Mue_t_lst[j], Identity)
            SigmaPlus_lst[j] = np.kron(SigmaPlus_lst[j], Identity)
            SigmaMinus_lst[j] = np.kron(SigmaMinus_lst[j], Identity)
# obtain the total electronic Hamiltonian
Ndim_e = 2**Ne
Ndim_e2 = Ndim_e * Ndim_e
Hme_expand = np.zeros((Ndim_e, Ndim_e))
Mue_expand = np.zeros((Ndim_e, Ndim_e))
Mue_t_expand = np.zeros((Ndim_e, Ndim_e))
for j in range(Ne):
    Hme_expand += Hme_lst[j]
    Mue_expand += Mue_lst[j] * e_dipole_cos_lst[j]
    Mue_t_expand += Mue_t_lst[j] * e_dipole_cos_lst[j]
Mue2_expand = np.dot(Mue_expand, Mue_expand)
...
def decode_initial_state(initial_state):
    Rho_e = initial_state[0:Ndim_e2].reshape((Ndim_e, Ndim_e))
...
Rho_e_expand = np.matrix([1.0])
for j in range(Ne):
    Rho_e_expand = np.kron(Rho_e_expand, Rho_e)
...
def func_dynamics(t, state):
    ...
    # equations of motion
    dxcdt = pc
    current_mue = np.trace(np.dot(Rho_e_expand, Mue_expand))
    current_muv = Nv ** 0.5 * xb * dv
    dpcdt = -omega_c**2 * xc - lambda_c * omega_c * (current_mue + current_muv) - gamma_c * pc
    Hsc_expand = Hme_expand + (omega_c * lambda_c * xc) * Mue_expand + (0.5 * lambda_c**2) * (Mue2_expand + 2.0 * Mue_expand * current_muv)
    # add the external pulse to excite the system
    Hsc_expand += gaussian_pulse(t) * Mue_t_expand
    dRhoedt_expand = -1j * np.dot(Hsc_expand, Rho_e_expand) + 1j * np.dot(Rho_e_expand, Hsc_expand)
    # Add a Lindblad term to describe the spontaneous emission and decoherence
    Lindblad = gamma_se * (np.dot(SigmaMinus_lst[0], np.dot(Rho_e_expand, SigmaPlus_lst[0])) - 0.5 * np.dot(SigmaPlus_lst[0], np.dot(SigmaMinus_lst[0], Rho_e_expand)) - 0.5 * np.dot(Rho_e_expand, np.dot(SigmaPlus_lst[0], SigmaMinus_lst[0])))
    if Ne > 1:
        for i in range(1, Ne):
            Lindblad += gamma_se * (np.dot(SigmaMinus_lst[i], np.dot(Rho_e_expand, SigmaPlus_lst[i])) - 0.5 * np.dot(SigmaPlus_lst[i], np.dot(SigmaMinus_lst[i], Rho_e_expand)) - 0.5 * np.dot(Rho_e_expand, np.dot(SigmaPlus_lst[i], SigmaMinus_lst[i])))
...
for t, y in zip(t_values, y_values):
    ...
    Pg_lst.append(np.real(Rho_e_expand[0, 0]))
    mu_current = np.real(np.trace(np.dot(Rho_e_expand, Mue_expand))) + Nv ** 0.5 * xb * dv
    e_ph = 0.5 * omega_c**2 * (xc + lambda_c / omega_c * mu_current)**2 + 0.5 * pc**2
    e_ph_lst.append(e_ph)
    e_Hme = np.real(np.trace(np.dot(Rho_e_expand, Hme_expand)))
